...
if(get_option("gigpress_target_blank") == 1) {
	$gigpress['target'] = ' target="_blank"';
	$gigpress['window_notice'] = "(". __("opens in a new window", "gigpress"). ")";
} else {
	$gigpress['target'] = "";
	$gigpress['window_notice'] = "";
}
		  ...
		  <tr>
			...
			<tr>
			  ...
			  <tr>
				...
				<br />
				<p class="gp-instructions"><?php _e("Use this space to list opening (or headlining) bands, &ldquo;presented by&rdquo; info, or whatever else you like.<br />Keep it brief, and maybe don&rsquo;t try to display images inline in this field.", "gigpress"); ?></p></td>
			  </tr>
			  <tr>
...
function gigpress_admin_table($shows, $type, $scope) {
	...
	foreach($shows as $show) {
		...
		if(!empty($show->show_venue_url)) {
			$venue = "<a href=\"$venue_url\"".$gigpress['target']." title=\"".$gigpress['window_notice']."\">".gigpress_sanitize($show->show_venue)."</a>";
		} else {
			$venue = gigpress_sanitize($show->show_venue);
		}
		...
		if(!empty($show->show_tix_url)) {
			$buy = "<a href=\"$tix_url\"".$gigpress['target']. "title=\"".$gigpress['window_notice']."\">". __("Link", "gigpress") ."</a>";
		} else {
...
function gigpress_add_show() {
	...
	if($errors) {
		...
		$addshow = $wpdb->query("
			INSERT INTO ". $gigpress['gigs_table'] ." (show_date, show_time, show_expire, show_multi, show_address, show_locale, show_country, show_price, show_tix_url, show_venue, show_venue_url, show_ages, show_notes, show_tour_id, show_related)
			VALUES ('". $date ."', '". $time ."', '". $expire ."', '". $multi ."','". $_POST['show_address'] ."', '". $_POST['show_locale'] ."', '". $country ."', '". $_POST['show_price'] ."', '". $_POST['show_tix_url'] ."', '". $_POST['show_venue'] ."', '". $_POST['show_venue_url'] ."', '". $_POST['show_ages'] ."', '". $_POST['show_notes'] ."', '". $tourid ."', '". $_POST['show_related'] ."')
		");
...
function gigpress_gigs_table($showarray, $type, $scope) {
	...
	foreach($showarray as $show) {
		...
		// if so, make magic happen with the clicking and such
		if(!empty($show->show_tix_url)) {
			$buy = "<a href=\"$tix_url\"".$gigpress['target']. "title=\"".$gigpress['window_notice']."\">". __("Buy tickets", "gigpress") ."</a>.";
		} else {
			$buy = "";
		}
		...
		<tr class="gigpress-info<?php echo $style; ?>">
			...
			<?php if($tour) { ?><span class="gigpress-info-item"><span class="gigpress-info-label"><?php _e("Tour", "gigpress"); ?>:</span> <?php echo gigpress_sanitize($tour).". " ?></span><?php } ?>
			...
			<?php if($show->show_related != 0) { ?><span class="gigpress-info-item"><?php echo gigpress_related_link($show->show_related); ?></span><?php } ?>
			</td>
		</tr>
		</tbody> <?php
	} // end foreach show
...
function gigpress_sidebar_content($shows, $type) {
	...
	foreach ($shows as $show) {
		...
		echo("</span> ".__("in", "gigpress")." <span class=\"summary\"><span class=\"hide\">". gigpress_sanitize(get_option('gigpress_band')) ." ".__("in", "gigpress")." </span>".gigpress_sanitize($show->show_locale)."</span> ".__("at", "gigpress")." <span class=\"location\">$venue</span></li>\n\t\t");
		$item++;
	} // end foreach show
}
...
function gigpress_mapit($address, $locale, $country) {
	global $gigpress;
	if($address) {
		$url = '<a href="http://maps.google.com/maps?&amp;q='. urlencode($address). ','. urlencode($locale) .','. urlencode($country) .'" title="'.__("Find this address using Google Maps.", "gigpress"). " " .$gigpress['window_notice']. '" class="gigpress-address"'. $gigpress['target']. '>'.$address.'</a>';
		return $url;
	}
}
