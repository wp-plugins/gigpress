...
$gigpress_db_version = "1.0";

// -------------------- ADMINISTRATION, BACK-END SHIZZLE ---------------------//

// See if our main table already exists, and if not,
// create both of our tables.  We shoud probably check for each individually
// - and concomitantly create each individually.  But alas.  We do not.

global $wpdb;
global $gigs_table; $gigs_table = $wpdb->prefix . "gigpress_shows";
global $tours_table; $tours_table = $wpdb->prefix . "gigpress_tours";

function gigpress_install () {
	global $wpdb;
	global $gigpress_db_version;
	global $gigs_table;
	global $tours_table;

	
	if($wpdb->get_var("SHOW tables LIKE '$gigs_table'") != $gigs_table) {
		$create = "CREATE TABLE " . $gigs_table . " (
		...
		ALTER TABLE " . $shows_table . " ADD FOREIGN KEY (show_tour_id) REFERENCES " . $tours_table . " (tour_id)
		;";
		...
		add_option("gigpress_db_version", $gigpress_db_version);
...
function gigpress_add() {
	...
	<?php 	// We're about to edit an existing show ...
		...
		SELECT * from ". $gigs_table ." WHERE show_id = ". $_GET['show_id'] ." LIMIT 1
		");
			if($showdata) {
				...
				foreach($showdata as $show) {
					$date = explode('-', $show->show_date);
						...
						$dd = $date[2];
						$yy = $date[0];
					$locale = $show->show_locale;
					$address = $show->show_address;
					$venue = $show->show_venue;
					$venue_url = $show->show_venue_url;
					$country = $show->show_country;
					$admittance = $show->show_ages;
					$tixurl = $show->show_tix_url;
					$notes = $show->show_notes;
		  ...
		  <tr>
			...
			<tr>
			  ...
			  <tr>
				...
				<?php // We'll stack what we think are the most often-used countries at the top, and make them "sticky" for when you're editing.  Too lazy to make the other 143 countries sticky as well. */ ?>
			  ...
			  <tr>
				...
				<td><select name="show_tour_id" id="show_tour_id">
				  ...
				  <?php $tours = $wpdb->get_results("
						SELECT * FROM ". $tours_table ." ORDER BY tour_created DESC
						");
						if($tours != FALSE) {
...
function gigpress_admin_upcoming() {
	...
	<?php
		...
		$tours = $wpdb->get_results("SELECT * FROM ". $tours_table ." ORDER BY tour_created ASC");
		...
		if($tours != FALSE) {
			...
			foreach($tours as $tour) {
				...
				$findshows = $wpdb->get_results("
					SELECT * FROM ". $gigs_table ." 
					WHERE show_tour_id = ". $tour->tour_id ." 
					AND show_date >= NOW() 
				...
				<?php foreach($findshows as $show) {
					...
					$address = gigpress_mapit($show->show_address, $show->show_locale, $show->show_country);
					...
					if(!empty($show->show_venue_url)) {
						$venue = "<a href=\"$venue_url\">$show->show_venue</a>";
					} else {
						$venue = $show->show_venue;
					}
					...
					<tr<?php echo $style; ?>>
						...
						<td><?php echo $show->show_locale; ?></td>
						<td><?php echo $venue; ?></td>
						<td><?php echo $address; ?></td>
						<td><?php echo $show->show_country; ?></td>
						<td><?php echo $show->show_ages; ?></td>
						<td class="gp-centre"><?php echo $show->show_price; ?></td>
						<td><?php echo $buy; ?></td>
					...
					<?php if(!empty($show->show_notes)) { ?>
						<tr<?php echo $style; ?>>
							<td colspan="10" class="notes"><?php echo $show->show_notes; ?></td>
						</tr>
					<?php } ?>
					<?php } ?>
	...
	<?php
		...
		// We don't have shows of any kind to show you ?>
			<tr><td colspan="10"><strong><?php echo get_option('gigpress_noupcoming'); ?></strong></td></tr>
		<?php } ?>
		</table>
	<?php gigpress_footer() ?>
	</div>
...
function gigpress_tours() {
	...
	<?php if(isset($_GET['gpaction']) && $_GET['gpaction'] == "edit") {
		...
		SELECT * from ". $tours_table ." WHERE tour_id = ". $_GET['tour_id'] ." LIMIT 1
		");
		if($tourdata) {
	...
	<?php
		...
		$tours = $wpdb->get_results("SELECT * from ". $tours_table ." ORDER BY tour_created DESC");
		...
		if($tours) {
			...
			foreach($tours as $tour) {
				...
				if($n = $wpdb->get_var("
					SELECT count(*) FROM ". $gigs_table ." WHERE show_tour_id = ". $tour->tour_id ."
				")) {
					$count = $n;
				} else {
					$count = 0;
...
function gigpress_admin_past() {
	...
	<?php
		...
		// We don't have shows of any kind to show you ?>
			<tr><td colspan="10"><strong><?php echo get_option('gigpress_nopast'); ?></strong></td></tr>
		<?php } ?>
		</table>
	<?php gigpress_footer() ?>
	</div>
...
// Aarrrr, and here be the Options page ...
...
function gigpress_options() {
	...
	<table width="100%" cellspacing="2" cellpadding="5" class="optiontable editform gp-table">
		<tr>
			...
			<td>
				<input type="text" name="gigpress_band" value="<?php echo get_option('gigpress_band'); ?>" /><br />

				<small class="gp-instructions"><?php _e('Right now this is just used for hCalendar data.') ?></small>
			</td>
		</tr>	
		<tr>
		...
		<tr>
			...
			<td>
				<input type="text" name="gigpress_noupcoming" size="48" value="<?php echo get_option('gigpress_noupcoming'); ?>" />
			</td>
		</tr>	
		<tr>
			<th width="33%" scope="row"><?php _e('No past shows message:') ?></th>
			<td>
				<input type="text" name="gigpress_nopast" size="48" value="<?php echo get_option('gigpress_nopast'); ?>" />
			</td>
		</tr>	
		<tr>
	...
	<table width="100%" cellspacing="2" cellpadding="5" class="optiontable editform gp-table">
		<tr>
			...
			<td>
				<input type="text" name="gigpress_widget_heading" value="<?php echo get_option('gigpress_widget_heading'); ?>" />
			</td>
		</tr>	
		<tr>
		...
		<input type="hidden" name="page_options" value="gigpress_band,gigpress_css,gigpress_tour_heading,gigpress_date_format,gigpress_noupcoming,gigpress_nopast,gigpress_widget_number,gigpress_widget_heading" />
...
function gigpress_add_show() {
	...
	if($errors) {
		...
		$addshow = $wpdb->query("
			INSERT INTO ". $gigs_table ." (show_date, show_address, show_locale, show_country, show_price, show_tix_url, show_venue, show_venue_url, show_ages, show_notes, show_tour_id)
		...
		if($addshow != FALSE) {
			...
			//YES! ?>
			<div id="message" class="updated fade"><p><?php _e('Your show  in '. stripslashes($_POST['show_locale']) .' on '.  mysql2date(get_option('gigpress_date_format'), $date) .' was successfully added.'); ?></p></div>
			
	<?php } else {
			// NO? ?>
			<div id="message" class="error fade"><p><?php _e('Something ain\'t right - try again?'); ?></p></div>			
	<?php }
	}
}
...
function gigpress_update_show() {
	...
	if($errors) {
		...
		$updateshow = $wpdb->query("
			UPDATE ". $gigs_table ." SET
			show_date = '". $date ."',
			show_address = '". $_POST['show_address'] ."',
			show_locale = '". $_POST['show_locale'] ."',
			show_country = '". $_POST['show_country'] ."',
			show_price = '". $_POST['show_price'] ."',
			show_tix_url = '". $_POST['show_tix_url'] ."',
			show_venue = '". $_POST['show_venue'] ."',
			show_venue_url = '". $_POST['show_venue_url'] ."',
			show_ages = '". $_POST['show_ages'] ."',
		...
		if($updateshow != FALSE) {
			// YES! ?>
				<div id="message" class="updated fade"><p><?php _e('Your show  in '. stripslashes($_POST['show_locale']) .' on '. mysql2date(get_option('gigpress_date_format'), $date) .' was successfully updated.'); ?></p></div>
		<?php	
		} else {
			// NO? ?>
				<div id="message" class="error fade"><p><?php _e('Something ain\'t right - try again?'); ?></p></div>
...
function gigpress_delete_show() {
	...
	$trashshow = $wpdb->query("
		DELETE FROM ". $gigs_table ." WHERE show_id = ". $_GET['show_id'] ." LIMIT 1
		");
	if($trashshow != FALSE) { ?>
...
function gigpress_add_tour() {
	...
	if(empty($_POST['tour_name']))  {
		...
		// Looks like we're all here, so let's add to the DB
		$addtour = $wpdb->query("
			INSERT into ". $tours_table ." (tour_created, tour_name)
		...
		if($addtour != FALSE) {
			...
			//YES! ?>
				<div id="message" class="updated fade"><p><?php _e('&ldquo;'. $_POST['tour_name'] .'&rdquo; was successfully added to the database.'); ?></p></div>
...
function gigpress_update_tour() {
	...
	if(empty($_POST['tour_name']))  {
		...
		$updatetour = $wpdb->query("
			UPDATE ". $tours_table ." SET
			tour_name = '". $_POST['tour_name'] ."'
			WHERE tour_id = ". $_POST['tour_id']."
		");
		
		// Was the query successful?
		if($updatetour != FALSE) {
			// YES! ?>
				<div id="message" class="updated fade"><p><?php _e('Tour name successfully changed to &ldquo;'. stripslashes($_POST['tour_name']) .'&rdquo;'); ?></p></div>
	<?php } else { // NO? ?>
				<div id="message" class="error fade"><p><?php _e('Something ain\'t right - try again?'); ?></p></div>
	<?php }
	}
...
function gigpress_delete_tour() {
	...
	$trashtour = $wpdb->query("
		DELETE FROM ". $tours_table ." WHERE tour_id = '". $_GET['tour_id'] ."' LIMIT 1;
		");
	if($trashtour != FALSE) {
		// Find any shows associated with that tour and remove their foreign key
		$cleanup = $wpdb->query("
		UPDATE ". $gigs_table ." SET show_tour_id = 0 WHERE show_tour_id = ". $_GET['tour_id'] .";
		");
...
function gigpress_footer() {
	echo('<p class="gigpress-footer"><a href="http://gigpress.com">You&rsquo;re using GigPress 1.0</a></p>');
}
...
function gigpress_upcoming($content) {
	...
	global $gigs_table;
	global $tours_table;	
	$heading = get_option('gigpress_tour_heading');
			...
			foreach($tours as $tour) {
				...
				$output.= '<tr>
					<th colspan="4" class="gigpress-heading">
						<'. $heading .'>'. $tour->tour_name .'</'. $heading .'>
					</th>
				</tr>';
				...
				foreach($findshows as $show) {
					...
					<tr class="gigpress-row'. $style .'">
						...
						<td class="gigpress-city summary"><span class="hide">'. get_option('gigpress_band') .' in </span>'. $show->show_locale .'</td>
					...
					<tr class="gigpress-info'. $style .'">
						...
						if($show->show_price) { $output.= ' <span class="gigpress-info-label">'. __('Admission:'). '</span> '. $show->show_price .'. '; }
	...
	// We don't have shows of any kind to show you
		$output.= '<tr><td colspan="4" class="gigpress-row">'. get_option('gigpress_noupcoming') .'</td></tr>';
	}
	$output.= '</table>';
...
function gigpress_archive($content) {
	...
	// We don't have shows of any kind to show you
		$output.= '<tr><td colspan="4" class="gigpress-row">'. get_option('gigpress_nopast') .'</td></tr>'; 
	}
	$output.= '</table>';
...
function gigpress_sidebar($number = 3) {
	...
	if($list != FALSE) {
		...
		foreach($list as $show) {
			...
			echo("\"><span class=\"gigpress-date\"><abbr class=\"dtstart\" title=\"$hdate\">$date</abbr></span><br /><span class=\"summary\"><span class=\"hide\">". get_option('gigpress_band') ." in </span>$show->show_locale</span> at <span class=\"location\">$venue</span></li>\n\t\t");
			$item++;
			}
	} else {
		echo('<li>'. get_option('gigpress_noupcoming') .'</li>');
	}
	
	echo('</ul>');
}

// Now for our widget users! 
...
function gigpress_widget_init() {
	...
	function gigpress_widget($args) {
		...
		$title = get_option('gigpress_widget_heading');
...
// Lastly - let's register our hooks and actions people!
