	...
	if($segment == 1) {
		...
		if($tours != FALSE) {
			...
			foreach($tours as $tour) {
				...
				// See if each tour actually has any shows assigned to it
				$shows = $wpdb->get_results("
					SELECT * FROM ". $gigpress['gigs_table'] ." WHERE show_tour_id = ". $tour->tour_id ." AND show_expire >= '". $now ."' AND show_status = 'active' ORDER BY show_date ASC LIMIT ".$number."
				");
		...
		$list = $wpdb->get_results("
			SELECT * FROM ". $gigpress['gigs_table'] ." WHERE show_expire >= '". $now ."' AND show_status = 'active' ORDER BY show_date ASC LIMIT ". $number ."
		");
...
function gigpress_feed() {
	...
	$shows = $wpdb->get_results("
		SELECT * FROM ". $gigpress['gigs_table'] ." WHERE show_expire >= '". $now ."' AND show_status = 'active' ORDER BY show_date ASC
	");
