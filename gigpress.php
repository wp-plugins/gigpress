...
function gigpress_install() {
	...
	if($installed_db != $gigpress['db_version']) {
		...
		// We need to do some stuff if they're moving from DB version 1.0 --> 1.1
		if($installed_db == "1.0") {
			...
			// These options are new in 1.2, and should be populated by default
			add_option("gigpress_user_level", "2");
			add_option("gigpress_display_country", "1");
			add_option("gigpress_time_format", "g:ia");
			add_option("gigpress_default_country", "US");
		
		} // End upgrade from DB version 1.0 --> 1.1
...
function gigpress_add() {
	...
	<?php } ?>
		...
		<table width="100%" cellspacing="2" cellpadding="5" class="optiontable editform gp-table">
		  <tr>
			<th width="33%" scope="row"><label for="mm"><strong><?php _e("Date", "gigpress") ?>:</strong></label></th>
			<?php 
			
			// If we're editing a show ...
			...
			<td><select name="mm" id="mm">
				...
				<tr id="expire"<?php if($multi != 1) echo(' class="inactive"'); ?>>
					<th width="33%" scope="row"><label for="mm"><strong><?php _e("End date", "gigpress") ?>:</strong></label></th>
			...
			<tr>
				<th width="33%" scope="row"><label for="show_locale"><strong><?php _e("City", "gigpress") ?>:</strong></label></th>
			  ...
			  <tr>
				<th width="33%" scope="row"><label for="show_venue"><strong><?php _e("Venue", "gigpress") ?>:</strong></label></th>
			  ...
			  <tr>
				<th width="33%" scope="row"><label for="show_venue_url"><?php _e("Venue website", "gigpress") ?>:</label></th>
			  ...
			  <tr>
				<th width="33%" scope="row"><label for="show_address"><?php _e("Address", "gigpress") ?>:</label></th>
			  ...
			  <tr>
				<th width="33%" scope="row"><label for="show_country"><?php _e("Country", "gigpress") ?>:</label></th>
				
				<?php if($_GET['gpaction'] != "edit") { $country = get_option('gigpress_default_country'); } ?>

				<td><select name="show_country" id="show_country">
						<?php gigpress_countries($country); ?>
					</select>
			</td>
			  </tr>
			  <tr>
				<th width="33%" scope="row"><label for="show_ages"><?php _e("Admittance", "gigpress") ?>:</label></th>
			  ...
			  <tr>
				<th width="33%" scope="row"><label for="show_price"><?php _e("Price", "gigpress") ?>:</label></th>
			  ...
			  <tr>
				<th width="33%" scope="row"><label for="show_tix_url"><?php _e("Ticket URL", "gigpress") ?>:</label></th>
			  ...
			  <tr>
				<th width="33%" scope="row"><label for="show_notes"><?php _e("Notes", "gigpress") ?>:</label></th>
				...
				Keep it brief, and maybe don&rsquo;t try to display images inline in this field.", "gigpress"); ?></p></td>
			  </tr>
			  <tr>
				<th width="33%" scope="row"><label for="show_tour_id"><?php _e("Part of a tour?", "gigpress") ?></label></th>
		  ...
		  <tr>
			...
			<td>
			<?php if(isset($_GET['gpaction']) && $_GET['gpaction'] == "edit") { ?>
				<p class="submit"><input type="submit" name="Submit" value="<?php _e("Update show", "gigpress") ?>" /></p></td>
			<?php } else { ?>
				<p class="submit"><input type="submit" name="Submit" value="<?php _e("Add show", "gigpress") ?>" /></p></td>
			<?php } ?>
		</tr>
		</table>		
...
function gigpress_admin_upcoming() {
	...
	<?php
		...
		if($tours != FALSE) {
			...
			foreach($tours as $tour) {
				...
				$shows = $wpdb->get_results("
					SELECT * FROM ". $gigpress['gigs_table'] ." WHERE show_tour_id = ". $tour->tour_id ." AND show_expire >= '". $now ."' ORDER BY show_date ASC
				");
				...
				<tr>
					<th colspan="10" class="gp-thead"><h3><?php _e("Tour", "gigpress"); echo ': ' . $tour->tour_name; ?></h3></th>
				</tr>
		...
		$upcoming = $wpdb->get_results("
			SELECT * FROM ". $gigpress['gigs_table'] ." WHERE show_expire >= '". $now ."' AND show_tour_id = 0 ORDER BY show_date ASC");
		...
		if($upcoming != FALSE) {
			...
			if($tourshows == TRUE) { ?>
			<tr>
				<th colspan="10" class="gp-thead"><h3><?php _e("Individual shows", "gigpress"); ?></h3></th>
			</tr>
	...
	<?php
		...
		if($tours != FALSE) {
			...
			foreach($tours as $tour) {
				...
				// See if each tour actually has any shows assigned to it
				$shows = $wpdb->get_results("
					SELECT * FROM ". $gigpress['gigs_table'] ." WHERE show_tour_id = ". $tour->tour_id ." AND show_expire < '". $now ."' ORDER BY show_date DESC
				");
		...
		$past = $wpdb->get_results("
			SELECT * FROM ". $gigpress['gigs_table'] ." WHERE show_expire < '". $now ."' AND show_tour_id = 0 ORDER BY show_date DESC");
...
function gigpress_admin_table($shows, $type, $scope) {
	...
	foreach($shows as $show) {
		...
		<tr<?php echo $style; ?>>
			...
			<td><?php echo $buy; ?></td>
			<td><a href="<?php echo $edit; ?>" class="edit"><?php _e("Edit", "gigpress"); ?></a></td>
			<td><a href="<?php echo $delete; ?>" class="delete"><?php _e("Delete", "gigpress"); ?></a></td>
		</tr>
...
function gigpress_tours() {
	...
	<p><?php _e("A tour is simply a collection of shows that you want to group together.  When displayed on your site, tours are grouped together under their respective headings.", "gigpress"); ?></p>
	...
	<?php if(isset($_GET['gpaction']) && $_GET['gpaction'] == "edit") {
		...
		<table width="100%" cellspacing="2" cellpadding="5" class="optiontable editform gp-table">
			<tr>
				<th width="33%" scope="row"><label for="tour_name"><?php _e("Edit your tour name", "gigpress") ?>:</label></th>
			...
			<tr>
				<th width="33%" scope="row">&nbsp;</th>
				<td>
				<p class="submit"><input type="submit" name="Submit" value="<?php _e("Update tour", "gigpress") ?>" /></p></td>
			</tr>
		</table>
		...
		<table width="100%" cellspacing="2" cellpadding="5" class="optiontable editform gp-table">
			<tr>
				<th width="33%" scope="row"><label for="tour_name"><?php _e("Add a new tour", "gigpress") ?>:</label></th>
			...
			<tr>
				<th width="33%" scope="row">&nbsp;</th>
				<td>
				<p class="submit"><input type="submit" name="Submit" value="<?php _e("Add tour", "gigpress") ?>" /></p></td>
			</tr>
		</table>
	...
	<table class="widefat">
		<thead>
			<tr>
				<th scope="col"><?php _e("Date added", "gigpress"); ?></th>
...
function gigpress_options() {
	...
	<table width="100%" cellspacing="2" cellpadding="5" class="optiontable editform gp-table">
		<tr>
			<th width="33%" scope="row"><?php _e("Name of your band", "gigpress") ?>:</th>
			<td>
		...
		<tr>
			<th width="33%" scope="row"><?php _e("User level required to use GigPress", "gigpress") ?>:</th>
			<td>
		...
		<tr>
			<th>&nbsp;</th>
			<td><p class="submit"><input type="submit" name="Submit" value="<?php _e("Update Options", "gigpress") ?>" /></p></td>
		</tr>
	</table>
	...
	<fieldset class="options" id="gpformatting">
		...
		<tr>
			<th width="33%" scope="row"><?php _e("Date format", "gigpress") ?>:</th>
			<td>
				<input type="text" name="gigpress_date_format" value="<?php echo get_option('gigpress_date_format'); ?>" />
			</td>
		</tr>	
		<tr>
			<th width="33%" scope="row"><?php _e("Time format", "gigpress") ?>:</th>
			<td>
		...
		<tr>
			<th width="33%" scope="row"><?php _e("Full URL to your &lsquo;Upcoming Shows&rsquo; page", "gigpress") ?>:</th>
			<td>
		...
		<tr>
			<th width="33%" scope="row"><?php _e("No upcoming shows message", "gigpress") ?>:</th>
			<td>
				<input type="text" name="gigpress_noupcoming" size="48" value="<?php echo gigpress_sanitize(get_option('gigpress_noupcoming')); ?>" />
			</td>
		</tr>	
		<tr>
			<th width="33%" scope="row"><?php _e("No past shows message", "gigpress") ?>:</th>
			<td>
		...
		<tr>
			<th width="33%" scope="row"><?php _e("RSS Feed", "gigpress") ?></th>
			<td>
				...
				<p class="gp-instructions"><?php _e("Your RSS feed for upcoming shows is", "gigpress"); ?><code><?php echo ' ' . $gigpress['rss']; ?></code>.</p>
			</td>
		</tr>
		<?php $country = get_option('gigpress_default_country'); ?>
		<tr>
			<th width="33%" scope="row"><?php _e("Default Country", "gigpress") ?>:</th>
			<td>
				<select name="gigpress_default_country">
					<?php gigpress_countries($country); ?>
				</select>
		...
		<tr>
			<th width="33%" scope="row"><?php _e("Heading level of tour names", "gigpress") ?>:</th>
			<td>
		...
		<tr>
			<th width="33%" scope="row"><abbr title="<?php _e("Cascading Style Sheets", "gigpress"); ?>"><?php _e("CSS", "gigpress") ?></abbr></th>
			<td>
				<p><label>
					<input name="gigpress_css" type="radio" value="gigpress" class="tog"<?php if(get_option('gigpress_css') == "gigpress") echo ' checked="checked"'; ?> />
					<?php _e("Use GigPress&rsquo; default style sheet to display your tour dates", "gigpress"); ?>
				</label></p>
				<p><label>
					<input name="gigpress_css" type="radio" value="user" class="tog"<?php if(get_option('gigpress_css') == "user") echo ' checked="checked"'; ?> />
					<?php _e("Use your own custom styles to display your tour dates", "gigpress"); ?>
				</label><br />
				<small class="gp-instructions"><?php _e("For more information on using your own style sheet,", "gigpress"); ?> <a href="http://gigpress.com/docs#css"><?php _e("please refer to the documentation", "gigpress"); ?></a>.</small>
				</p>
			</td>
		</tr>
	...
	<table width="100%" cellspacing="2" cellpadding="5" class="optiontable editform gp-table">
		<tr>
			<th width="33%" scope="row"><?php _e("List heading", "gigpress") ?>:</th>
			<td>
				<input type="text" name="gigpress_widget_heading" value="<?php echo gigpress_sanitize(get_option('gigpress_widget_heading')); ?>" />
			</td>
		</tr>
		<tr>
			<th width="33%" scope="row"><?php _e("Number of shows to list", "gigpress") ?>:</th>
			<td>
		...
		<tr>
			<th width="33%" scope="row"><?php _e("Segment sidebar list into tours?", "gigpress") ?></th>
			<td>
...
function gigpress_add_show() {
	...
	if($errors) {
		...
		$addshow = $wpdb->query("
			...
			VALUES ('". $date ."', '". $time ."', '". $expire ."', '". $multi ."','". $_POST['show_address'] ."', '". $_POST['show_locale'] ."', '". $_POST['show_country'] ."', '". $_POST['show_price'] ."', '". $_POST['show_tix_url'] ."', '". $_POST['show_venue'] ."', '". $_POST['show_venue_url'] ."', '". $_POST['show_ages'] ."', '". $_POST['show_notes'] ."', '". $_POST['show_tour_id'] ."')
		");
...
function gigpress_delete_show() {
	...
	$trashshow = $wpdb->query("
		...
		");
	if($trashshow != FALSE) { ?>
			<div id="message" class="updated fade"><p><?php _e("Show successfully deleted from the database.", "gigpress") ?></p></div>
	<?php } else { ?>
...
function gigpress_delete_tour() {
	...
	if($trashtour != FALSE) {
		...
		"); ?>
		<div id="message" class="updated fade"><p><?php _e("Tour successfully deleted from the database.", "gigpress"); ?></p></div>
	<?php } else { ?>
...
function gigpress_upcoming_content() {
	...
	<tr>
		<td class="gigpress-rss gigpress-row" colspan="<?php echo $cols; ?>"><a href="<?php echo $gigpress['rss']; ?>" rel="alternate" type="application/rss+xml" title="<?php _e("Upcoming shows RSS feed", "gigpress"); ?>">RSS</a></td>
	</tr>
	<?php } ?>
	</table>
...
function gigpress_shows_table($shows, $type, $scope) {
	...
	foreach($shows as $show) {
		...
		<tr class="gigpress-info<?php echo $style; ?>">
			...
			<?php if($secs != "1") { ?>
				<span class="gigpress-info-label"><?php _e("Time", "gigpress"); ?>:</span> <?php echo $time.". " ?>
			<?php } ?>
			<?php if($show->show_price) { ?>
				<span class="gigpress-info-label"><?php _e("Admission", "gigpress"); ?>:</span> <?php echo gigpress_sanitize($show->show_price) .'. ';
			} ?>
			<?php if($show->show_ages != "Not sure") { ?>
				<span class="gigpress-info-label"><?php _e("Age restrictions", "gigpress"); ?>:</span> <?php echo gigpress_sanitize($admittance) .'. ';
			} ?>
			<?php if($scope == "upcoming") { echo $buy; } ?>
			<?php if($address) { ?> <span class="gigpress-info-label"><?php _e("Address", "gigpress"); ?>:</span> <?php echo $address .'. '; } ?>
...
function gigpress_sidebar_content($shows, $type) {
	...
	foreach ($shows as $show) {
		...
		echo("</span><br /><span class=\"summary\"><span class=\"hide\">". gigpress_sanitize(get_option('gigpress_band')) ." in </span>".gigpress_sanitize($show->show_locale)."</span> ".__("at", "gigpress")." <span class=\"location\">$venue</span></li>\n\t\t");
		$item++;
	} // end foreach show
}
...
function gigpress_sidebar($number = 3,$segment = 1) {
	...
	if($segment == 1) {
		...
		$list = $wpdb->get_results("
			SELECT * FROM ". $gigpress['gigs_table'] ." WHERE show_expire >= '". $now ."' AND show_tour_id = 0 ORDER BY show_date ASC LIMIT ". $number ."
		");
	...
	echo('<li class="gigpress-list-rss"><a href="'.$gigpress['rss'].'" rel="alternate" type="application/rss+xml" title="'.__("Upcoming shows RSS feed", "gigpress").'">RSS</a></li>');
	}
	echo("\n\t\t</ul>");
...
function gigpress_countries($country) { ?>
	...
	if($shows != FALSE) {
		foreach ($shows as $show) {
			...
			echo('<item>
				...
				echo $buy;
				echo('Venue: '.$venue.'. '); if($address) { echo(__("Address", "gigpress").': '.$address.'. '); }
...
function gigpress_mapit($address, $locale, $country) {
	if($address) {
		$url = '<a href="http://maps.google.com/maps?&amp;q='. urlencode($address). ','. urlencode($locale) .','. urlencode($country) .'" title="'.__("Find this address using Google Maps.", "gigpress").'" class="gigpress-address">'.$address.'</a>';
		return $url;
	}
}
